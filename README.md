...
**🔗 https://api.neomovies.ru/docs**

Особенности:
- **Scalar API Reference** - современный интерфейс
...
После деплоя проверьте health check для диагностики:
```bash
...
API автоматически проверяет различные названия для MongoDB:
- `MONGO_URI` (рекомендуется)
- `MONGODB_URI` 
- `DATABASE_URL`
- `MONGO_URL`
...
2. Убедитесь что MongoDB URI внешний (не localhost)
...
**🎉 Готов к продакшену! Деплойте на Vercel и наслаждайтесь высокой производительностью Go! 🚀**
